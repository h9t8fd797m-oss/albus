// _shared/apple_roots.ts
//
// Apple's public root certificates, base64-encoded DER. These are the trust
// anchors for every App Store signed payload: a transaction is only believed
// if its certificate chain terminates here.
//
// Public data, safe to commit — this is the same file Apple serves at
// https://www.apple.com/certificateauthority/. Embedded rather than fetched
// so verification never depends on a network call that could be intercepted
// or simply fail at the wrong moment.

import { Buffer } from "node:buffer";

export const APPLE_ROOT_CA_G3_BASE64 =
  "";

export const APPLE_ROOT_CA_G2_BASE64 =
  "";

export function appleRootCertificates(): Buffer[] {
  return [APPLE_ROOT_CA_G3_BASE64, APPLE_ROOT_CA_G2_BASE64]
    .map((b64) => Buffer.from(b64, "base64"));
}
