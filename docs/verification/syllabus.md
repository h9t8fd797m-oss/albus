...
The existing database workflow now resets and seeds the corpus and runs `prove_topics_db_guards.py`. It expects the actual pgTAP failure for each invalid row fixture, then requires the clean corpus to pass after rollback. It also asserts that seed data exists, preventing an empty table from making both invariants vacuously green. Local blocked-command output is captured in syllabus-db-blocker.txt; CI results will supply the database evidence if its runner succeeds.
